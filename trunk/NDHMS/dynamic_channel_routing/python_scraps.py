...
#ADD NEIGHBOR Concept
    ...
    def add_upstream_boundary_condition_time_step(self, upstream_flow):
        self.time_steps.append(self.TimeStep(new_flow = upstream_flow))

    def add_downstream_boundary_condition_time_step(self, downstream_depth):
        self.time_steps.append(self.TimeStep(new_depth = downstream_depth))

    def add_time_step(self, new_flow):
        #print(self.bottom_width, self.manning_n_ds, self.bottom_z, self.bed_slope_ds, new_flow)
        new_depth = y_direct(self.bottom_width, self.manning_n_ds, self.bed_slope_ds, new_flow)
        self.time_steps.append(self.TimeStep(new_flow, new_depth))
...
def input_and_initialize(section_arr, input_path=None, input_opt=1, output_path=None):
    input_data = {}
    if input_opt == 1:
...
def main():
    ...
    sections = []
    input_opt = 0 #generate input in-line rather than from files (useful for testing)
    input_and_initialize(section_arr = sections, input_opt = input_opt)
...
def main():
    ...
    for j in range(ntim-1):
        ...
        we are properly reading input and plotting it'''
        '''
        for i, section in enumerate(sections):
            new_flow = sections[0].time_steps[j].flow
            #print(j, i, new_flow)
            section.add_time_step(new_flow)

    cols_for_subplots = 2
    fig, axes = plt.subplots(nrows=ceil(len(sections)/cols_for_subplots), ncols=cols_for_subplots, squeeze=False)
    for i, section in enumerate(sections):
        ...
        sections[0].time_steps[j].delta_flow_predictor = sections[0].time_steps[j+1].flow - sections[0].time_steps[j].flow
