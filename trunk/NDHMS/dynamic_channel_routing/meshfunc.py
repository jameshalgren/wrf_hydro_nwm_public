...
def matrixp(section_arr
    ...
    for i, section in enumerate(section_arr):
        ...
        wetted_perimeter = section.get_wetted_perimeter_area(section_j.flow_area)
...
def secpred(section_arr
    ...
    #        co(i)=sk(i)*areap(i)*hy**(2.0/3.0)
        ...
        #C       write(*,*)d(i),flow_area(i),ci1(i),y(j,i),z(i)
        #TODO: Where Does this WRITE TO???
        if i == 0 and not(secpred): #treat the downstream boundary as a special case
            continue
        else:
            if i == 0: print('HERE!')
            section_US = section_arr[i-1]
...
def matrixc(section_arr
  ...
  #      ter=bo(i)+2.0*areap(i)/bo(i)
        wetted_perimeter = section.get_wetted_perimeter_area(area)
  ...
  #    1     (areap(i)**(5.0/3.0)*2.0/bo(i)))/ter/ter
        dkda = 1 / section.manning_n_ds \
            * (( 5.0 / 3.0 * area ** (2.0/3.0) * wetted_perimeter) \
            - (area ** (5.0/3.0) * 2.0 / section.bottom_width)) \
            / (wetted_perimeter ** 2.0)
        #TODO: not compatible with generalized sections.
        st11 = 0.0
        st12 = 0.0
        st21 = constants.GRAVITY * area/section.bottom_width \
            / section.bottom_width * section.dbdx_ds + section_j.gs0_ds \
            + constants.MANNING_M * 2.0 * constants.GRAVITY \
            * area * flow \
            * abs (flow) / section_j.conveyance ** 3.0 \
            * dkda
        st22 = -2.0 * constants.MANNING_M * area \
             * area \
             / section_j.conveyance
      #c     Matrix S (eq 14)
      #      st11=0.0
      ...
      # IN SECTION CASE, we compute dx from i, in the secpred/matrixc case, dx from i-1 for section/matrixp
      # ALSO, the order of this operation is reversed 
      #        cour=dt(i)
      #      else
      #        cour=dt(i)/dx(i-1)
      #      endif
      ...
      #        crmin=min(crmin,cour*max(d11,d22))
      #      endif
        '''END COME BACK TO THIS'''

        if secpred:
            thessign = 1.0
        else:
            thessign = -1.0
        g11 = 0.5 + meshconstants.PHI + meshconstants.THETA * cour * a11 + thessign * 0.5 * meshconstants.THETAS * st11 * dt
        g12 = meshconstants.THETA * cour * a12 + thessign * 0.5 * meshconstants.THETAS * st12 * dt
        g21 = meshconstants.THETA * cour * a21 + thessign * 0.5 * meshconstants.THETAS * st21 * dt
        g22 = 0.5 + meshconstants.PHI + meshconstants.THETA * cour * a22 + thessign * 0.5 * meshconstants.THETAS * st22 * dt
